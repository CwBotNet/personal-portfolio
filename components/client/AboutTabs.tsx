...
const AboutTabs = async () => {

    try {
        const response: any = await httpModule.get<bio[]>('/bio')
        const data = response.data.data[0]
        // console.log(data.bio as string);
        ...
        const tabs = [
            {
                ...
                content: (
                    <div className="h-full w-full dark:bg-black bg-white  dark:bg-dot-white/[0.2] bg-dot-black/[0.2] relative flex items-center justify-center rounded-2xl p-10 text-xl md:text-4xl font-bold text-white">
                        ...
                        {/* <p>{data.bio}</p> */}
                        <TextGenerateEffect className="text-center text-wrap tracking-wide container text-white/85" words={`${data.bio}`}
                        />
                    </div>
                ),
            },
